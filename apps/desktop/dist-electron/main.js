...
const rootDir = join(__dirname$1, "..");
...
function loadConfig() {
  ...
  try {
    ...
    if (fs.existsSync(projectConfigPath)) {
      const fileConfig = load(fs.readFileSync(projectConfigPath, "utf8"));
  ...
  try {
    if (fs.existsSync(userConfigPath)) {
      const fileConfig = load(fs.readFileSync(userConfigPath, "utf8"));
      config = { ...config, ...fileConfig };
    } else {
      fs.writeFileSync(userConfigPath, dump(config), "utf8");
...
function saveConfig(config) {
  try {
    ...
    fs.writeFileSync(userConfigPath, dump(configToSave), "utf8");
...
function getSafeConfig(config) {
  ...
  if (safeConfig.openaiApiKey) {
    safeConfig.openaiApiKey = safeConfig.openaiApiKey.substring(0, 3) + "..." + safeConfig.openaiApiKey.substring(safeConfig.openaiApiKey.length - 3);
  }
  return safeConfig;
}
class DbService {
